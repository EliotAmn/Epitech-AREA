    ...
    static register(): DynamicModule {
        ...
        for (const dir of dirs) {
            ...
            try {
                ...
                const svc = importedService.default ?? importedService[Object.keys(importedService)[0]];
                if (svc) discoveredServices.push(svc);
                else console.warn(`[service] ${dir.name} ignored (no valid service export found)`);
            } catch (errService) {
                try {
        ...
        console.log(`[service] Discovered services: ${discoveredServices.map(svc => svc.name).join(', ')}`);
        return {
