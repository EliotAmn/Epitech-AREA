...
export class DiscordClientManager {
    ...
    private memberJoinHandlers: Map<string, (member: GuildMember) => void> = new Map();
    ...
    public async initialize(token: string): Promise<void> {
        ...
        this.client.on(Events.MessageCreate, (message) => {
            ...
            console.log(`📨 Message received from ${message.author.username}: ${message.content.substring(0, 50)}${message.content.length > 50 ? '...' : ''}`);
            console.log(`   Channel: ${message.channel.type === 1 ? 'DM' : message.channel.type === 0 ? (message.channel as any).name : 'Unknown'}`);
            console.log(`   Active handlers: ${this.messageHandlers.size}`);
    ...
    public registerMessageHandler(id: string, handler: (message: Message) => void): void {
    ...
    public registerMemberJoinHandler(id: string, handler: (member: GuildMember) => void): void {
