...
export class UpdateRecord extends ServiceReactionDefinition {
  ...
  async execute(
    ...
    try {
      const response = await axios.patch<AirtableUpdateResponse>(
      ...
      logger.log(`Successfully updated record: ${response.data.id}`);
    } catch (error: unknown) {
      const err = error as { response?: { status?: number; data?: unknown } };
      logger.error(
        `Failed to update record: ${JSON.stringify(err.response?.data)}`,
      );
      throw new Error(`Failed to update record: ${err.response?.status}`);
    }
  }
