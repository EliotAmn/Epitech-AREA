...
export default function CatalogPage({ items, description, onSelect }: CatalogPageProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (it) =>
        ...
        it.plateforme.toLowerCase().includes(q)
    );
  }, [items, query]);

  return (
    <div className="min-h-screen flex flex-col items-center px-4 sm:px-8">
      {description && <div className="mb-4 text-center text-4xl font-bold">{description}</div>}
